...
void InitPriorityQue_onepass(CDEM& dem, Flag& flag, PriorityQueue& priorityQueue)
{
    // 获取DEM的宽度和高度  
...
// 处理追踪队列中的节点  
void ProcessTraceQue_onepass(CDEM& dem, Flag& flag, queue<Node>& traceQueue, PriorityQueue& priorityQueue)
{
...
// 处理洼地单元格  
void ProcessPit_onepass(CDEM& dem, Flag& flag, queue<Node>& depressionQue, queue<Node>& traceQueue, PriorityQueue& priorityQueue)
{
...
void FillDEM_Zhou_OnePass(const char* inputFile, const char* outputFilledPath)
{
    ...
    // 初始化优先级队列  
    InitPriorityQue_onepass(dem, flag, priorityQueue);
    // 处理优先级队列中的节点  
    while (!priorityQueue.empty())
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            if (iSpill <= spill)
            {
                ...
                ProcessPit_onepass(dem, flag, depressionQue, traceQueue, priorityQueue);
            }
            else
            {
                //slope cell
            ...
            ProcessTraceQue_onepass(dem, flag, traceQueue, priorityQueue);
        }
    }
    // 记录结束时间  
